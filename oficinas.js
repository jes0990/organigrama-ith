const oficinas = {

    direccion: {
        nombre: "DIRECCIÓN",
        titular: "Evaristo Rogaciano López Hernández",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Dirección general del instituto",
            "Supervisión de subdirecciones y departamentos",
            "Autorización institucional"
        ]
    },

    sub_academica: {
        nombre: "SUBDIRECCIÓN ACADÉMICA",
        titular: "Jacobo Antonio Cruz",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Gestión académica",
            "Supervisión docente",
            "Control del avance académico"
        ]
    },

    sub_planeacion: {
        nombre: "SUBDIRECCIÓN DE PLANEACIÓN Y VINCULACIÓN",
        titular: "Egleyde Gómez Nochebuena",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Planeación estratégica",
            "Vinculación institucional",
            "Coordinación de proyectos"
        ]
    },

    sub_admin: {
        nombre: "SUBDIRECCIÓN DE SERVICIOS ADMINISTRATIVOS",
        titular: "Juan de Dios Viniegra Vargas",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Gestión administrativa",
            "Control de recursos internos"
        ]
    },

    cbasicas: {
        nombre: "DEPARTAMENTO DE CIENCIAS BÁSICAS",
        titular: "Carlos Eduardo Rosas Álvarez",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Coordinación de materias básicas",
            "Control de academias",
            "Supervisión de docentes"
        ]
    },

    ingenierias: {
        nombre: "DEPARTAMENTO DE INGENIERÍAS",
        titular: "Eliceo Hernández Hernández",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Gestión de carreras de ingeniería",
            "Supervisión académica"
        ]
    },

    profesionales: {
        nombre: "DIVISIÓN DE ESTUDIOS PROFESIONALES",
        titular: "Lucas Hernández Hernández",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Control escolar",
            "Seguimiento académico",
            "Titulación"
        ]
    },

    acad_desarrollo: {
        nombre: "DEPARTAMENTO DE DESARROLLO ACADÉMICO",
        titular: "Juana Pérez Lara",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Capacitación docente",
            "Programación académica"
        ]
    },

    quimica: {
        nombre: "DEPTO. INGENIERÍA QUÍMICA Y BIOQUÍMICA",
        titular: "Fernando Ricardo Mendoza Paz",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Coordinación de carreras de química y bioquímica"
        ]
    },

 sistemas: {
    nombre: "DEPARTAMENTO DE SISTEMAS Y COMPUTACIÓN",
    titular: "Ioset Ivette Sandoval Vargas",
    horario: "9:00 AM – 3:00 PM",
    servicios: [
        "Gestión académica de ISC",
        "Supervisión de laboratorios",
        "Proyectos de software"
    ],
    laboratorios: {
        cantidad: 5,
        descripcion: "Laboratorios destinados exclusivamente a la carrera de Ingeniería en Sistemas Computacionales, organizados por semestres y grupos."
    },
    areas: [
        "Administración", "Auditoría", "Bases de datos", "Calidad del software",
        "Contabilidad", "Desarrollo sustentable", "Estadística",
        "Estructura de datos", "Programación", "Redes",
        "Sistemas de información", "Investigación de operaciones",
        "Matemáticas", "Mercadotecnia", "Probabilidad",
        "Sistemas operativos", "Seminario de ética",
        "Software de sistemas", "Psicología organizacional",
        "Taller de investigación", "Tópicos avanzados de bases de datos"
    ]
},
    economicas: {
        nombre: "DEPTO. ECONÓMICO ADMINISTRATIVAS",
        titular: "Luis Miguel Velasco Barragán",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Gestión de carreras administrativas"
        ]
    },

    comunicacion: {
        nombre: "DEPARTAMENTO DE COMUNICACIÓN Y DIFUSIÓN",
        titular: "Federico Anselry Cabrera Sánchez",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Difusión institucional",
            "Redes sociales",
            "Eventos"
        ]
    },

    planeacion: {
        nombre: "PLANEACIÓN, PROGRAMACIÓN Y PRESUPUESTACIÓN",
        titular: "Salvador Medecigo Castillo",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Planeación institucional",
            "Administración de presupuestos"
        ]
    },

    vinculacion: {
        nombre: "GESTIÓN TECNOLÓGICA Y VINCULACIÓN",
        titular: "Mayrely Nava Guzmán",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Vinculación con empresas",
            "Convenios",
            "Residencias profesionales"
        ]
    },

    extraescolares: {
        nombre: "DEPARTAMENTO DE ACTIVIDADES EXTRAESCOLARES",
        titular: "Víctor Zuñiga Lorenzana",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Actividades deportivas",
            "Actividades culturales"
        ]
    },

    escolares: {
        nombre: "DEPARTAMENTO DE SERVICIOS ESCOLARES",
        titular: "Zayra Esther López Orta",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Control de documentos",
            "Constancias",
            "Movilidad"
        ]
    },

    informacion: {
        nombre: "CENTRO DE INFORMACIÓN",
        titular: "Patricia Pozos Carrillo",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Consulta bibliográfica",
            "Préstamo de libros"
        ]
    },

    rrhh: {
        nombre: "DEPARTAMENTO DE RECURSOS HUMANOS",
        titular: "Rosi Areli Hernández Cruz",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Gestión de personal",
            "Nómina",
            "Contratos"
        ]
    },

    financieros: {
        nombre: "DEPARTAMENTO DE RECURSOS FINANCIEROS",
        titular: "Jesús Hernández Aguilera",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Pagos",
            "Control presupuestal"
        ]
    },

    materiales: {
        nombre: "DEPTO. RECURSOS MATERIALES Y SERVICIOS",
        titular: "Petra Martínez Espinosa",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Adquisiciones",
            "Servicios generales"
        ]
    },

    computo: {
        nombre: "CENTRO DE CÓMPUTO",
        titular: "Delfino Olivares Sagahón",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Laboratorios de cómputo",
            "Infraestructura TI"
        ]
    },

    mantenimiento: {
        nombre: "DEPTO. MANTENIMIENTO Y EQUIPO",
        titular: "Justino Garfias Maldonado",
        horario: "9:00 AM – 3:00 PM",
        servicios: [
            "Mantenimiento de equipos",
            "Reparaciones"
        ]
    }

};
